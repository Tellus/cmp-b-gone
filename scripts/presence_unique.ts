...
async function main(): Promise<void> {
  ...
  console.debug(`Parsing ${yamlDescriptorFiles.length} YAML descriptors in ${sourcePath}.`);
  ...
  for (const yamlDescriptorFile of yamlDescriptorFiles) {
    ...
    if (descriptor.selectors?.presence !== undefined && Array.isArray(descriptor.selectors?.presence)) {
      ...
      console.warn(`NO presence selectors in ${yamlDescriptorFile}!`);
      continue;
    }
    ...
    for (const presenceSelector of presence) {
      ...
      if (existing !== undefined) {
        // console.error(`Selector ${presenceSelector} already exists! (duplicate was encountered in ${yamlDescriptorFile})`);
        existing.push(yamlDescriptorFile);
        presenceSelectors.set(presenceSelector, existing);
      } else {
        // console.info(`Selector ${presenceSelector} is unique (so far).`);
  ...
  for (const e of presenceSelectors.entries()) {
    if (e[1].length > 1) {
      console.debug(`${e[0]} (${e[1].length}):`);
      e[1].forEach(e => console.debug(e));
    }
  }
}
